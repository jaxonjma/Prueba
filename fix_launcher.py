#!/usr/bin/env python3
"""Imperio Studio v1.18.10 - Launcher Repair Script
Run with: python3 fix_launcher.py
"""
import gzip, base64, os, sys, platform, shutil

LAUNCHER_DATA = (
    "H4sIAOJDsmkC/+087XLjRnL/+RQwUioBtxT0sas9HytMStaHrTqtpJJkOz5RhwKBoQgTBGAA1GcplYfI"
...
def get_install_dir():
    ...
    if system == "Windows":
        ...
        return os.path.join(base, "Batchy")
    elif system == "Darwin":
        return os.path.join(os.path.expanduser("~"), "Library", "Application Support", "Batchy")
    else:
        return os.path.join(os.path.expanduser("~"), ".batchy")

INSTALL_DIR = get_install_dir()
LAUNCHER_PATH = os.path.join(INSTALL_DIR, "batchy_launcher.py")

print("Imperio Studio - Launcher Repair (v1.18.10)")
print("=" * 44)
print(f"  Platform  : {platform.system()}")
print(f"  Install   : {INSTALL_DIR}")

if not os.path.exists(INSTALL_DIR):
    print(f"ERROR: Imperio Studio not found at {INSTALL_DIR}")
    sys.exit(1)

if os.path.exists(LAUNCHER_PATH):
    backup = LAUNCHER_PATH + ".bak"
...
print(f"  Replaced  : {LAUNCHER_PATH}")
print(f"  Size      : {len(launcher_bytes):,} bytes")
print()
print("Done! Open Imperio Studio - auto-update will work correctly.")
